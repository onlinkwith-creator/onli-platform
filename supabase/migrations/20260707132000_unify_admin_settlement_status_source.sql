...
set settlement_status = case
  when coalesce(settlement_status, payout_status) in ('settlement_confirmed', 'confirmed', '정산확정', '정산 확정') then 'settlement_confirmed'
  when coalesce(settlement_status, payout_status) in ('settlement_paying', 'payment_started', 'payment_in_progress', 'payout_started', 'paying', '지급중', '통역사지급', '통역사 지급') then 'settlement_paying'
  when coalesce(settlement_status, payout_status) in ('settlement_completed', 'settlement_done', 'payment_completed', 'payout_completed', 'completed', 'paid', 'done', '정산완료', '정산 완료', '지급완료') then 'settlement_completed'
  else 'settlement_waiting'
end;

update public.settlements
set payout_status = case
  when settlement_status = 'settlement_completed' then 'paid'
  when settlement_status in ('settlement_confirmed', 'settlement_paying') then 'confirmed'
  else 'pending'
end
where payout_status is distinct from case
  when settlement_status = 'settlement_completed' then 'paid'
  when settlement_status in ('settlement_confirmed', 'settlement_paying') then 'confirmed'
  else 'pending'
end;
...
begin
  ...
  select interpreter_id, settlement_status
  into v_interpreter_id, v_previous_status
  ...
  v_next_payout_status := case
    when v_next_status = 'settlement_completed' then 'paid'
    when v_next_status in ('settlement_confirmed', 'settlement_paying') then 'confirmed'
    else 'pending'
  end;

  update public.settlements
  set settlement_status = v_next_status,
      payout_status = v_next_payout_status,
      ...
      paid_at = case
        when v_next_status = 'settlement_completed' then coalesce(paid_at, now())
        when v_next_status = 'settlement_waiting' then null
        else paid_at
      end,
      updated_at = now()
